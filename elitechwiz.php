<?php
error_reporting(0);
require 'elitechwiz_functions.php';
require 'elitechwiz_var.php';
require 'lib/elitechwiz_scanner.php';
echo $cln;
ew_enforce_integrity(__DIR__);
function update()
    {
        echo "\n\e[91m\e[1m[+] ELITECHWIZ UPDATE UTILITY [+]\nUpdate in progress, please wait...\n\n$cln";
        system("git fetch origin && git reset --hard origin/master && git clean -f -d");
        echo $bold . $fgreen . "[i] Job finished successfully! Please restart elitechwiz \n" . $cln;
        exit;
    }
...
if (rh_cli_entrypoint($_SERVER['argv'] ?? [])) {
    exit;
}
...
elseif ($ip == "fix")
  {
    echo "\n\e[91m\e[1m[+] elitechwiz Fix Menu [+]\n\n$cln";
    ew_fix_dependencies();
    echo $bold . $fgreen . "[i] Job finished successfully! Please restart elitechwiz \n";
    exit;
  }
...
askscan:
    ...
    if (!in_array($scan, array(
    ...
    else
      {
        ...
        elseif ($scan == "7")
          {
            ...
            echo "\n$cln" . $lblue . $bold . "[+] Scanning Begins ... \n";
            echo $blue . $bold . "[i] Scanning Site:\e[92m $ipsl" . "$ip \n";
            echo $bold . $yellow . "[S] Scan Type : Subdomain Scanner (Enhanced)" . $cln;
            rh_run_subdomain_scan([
                'domain' => $lwwww,
                'show-each' => true,
                'show-progress' => false
            ], false);
        ...
        elseif ($scan == "8")
          {
            ...
            if (count($array) > 0)
              {
                userinput("Do You Want elitechwiz To Detect CMS Of The Sites? [Y/N]");
                ...
                if ($wp_version != "")
                  {
                    ...
                    if (strpos($vuln_json, "The page you were looking for doesn't exist (404)") !== false)
                      {
                        echo $red . "[!] Seems like the version detection messed up preety bad! Please report it in your elitechwiz issue tracker.\n";
                      }
                    else
                      {
                  ...
                  {
                    $reallink = $ipsl . $ip;
                    echo $red . "Failed \n\n[!] elitechwiz could not determine the WordPress version of the target!";
                    echo "\n\n";
                    echo $bold . $yellow . "[*] Scanning Complete. Press Enter To Continue OR CTRL + C To Stop\n\n";
                    trim(fgets(STDIN, 1024));
                    goto scanlist;
        ...
        elseif ($scan == "12")
          {
            ...
            if (file_exists("elitechwiz_crawl/admin.ini"))
              {
                echo $bold . $fgreen . "\n[^_^] Admin Crawler File Found! Scanning For Admin Pannel [-]\n" . $cln;
                $crawllnk = file_get_contents("elitechwiz_crawl/admin.ini");
            ...
            if (file_exists("elitechwiz_crawl/backup.ini"))
              {
                echo "\n[-] Backup Crawler File Found! Scanning For Site Backups [-]\n";
                $crawllnk = file_get_contents("elitechwiz_crawl/backup.ini");
                $crawls   = explode(',', $crawllnk);
                echo "\nURLs Loaded: " . count($crawls) . "\n\n";
                foreach ($crawls as $crawl)
                  {
            ...
            if (file_exists("elitechwiz_crawl/others.ini"))
              {
                echo "\n[-] General Crawler File Found! Crawling The Site [-]\n";
                $crawllnk = file_get_contents("elitechwiz_crawl/others.ini");
                $crawls   = explode(',', $crawllnk);
                echo "\nURLs Loaded: " . count($crawls) . "\n\n";
                foreach ($crawls as $crawl)
                  {
        ...
        elseif ($scan == "14")
          {
            echo "\n$cln" . $lblue . $bold . "[+] Zero Rate Host Scan Mode (Educational)\n" . $cln;
            rh_run_zero_rate_scan([], false);
            goto scanlist;
          }
        elseif ($scan == 'U' || $scan == 'u')
          {
            echo "\n\n" . $bold . $yellow . "-[ elitechwiz Update Corner]-\n\n" . $cln;
            echo $bold . "[i] Fetching Stuffs .... \n" . $cln;
            $latestversion = readcontents("elitechwiz_version.txt");
            ...
            elseif ($rhversion == $latestversion)
              {
                echo $bold . $fgreen . "\n[i] You are already running the latest version of elitechwiz. \n\n" . $cln;
                goto scanlist;
              }
            else
              {
        ...
        elseif ($scan == "A" || $scan == "a")
          {
            ...
            rh_run_subdomain_scan([
                ...
                'show-each' => true,
                'format' => 'csv',
                'output' => rh_default_output_path('subdomains', $lwwww, 'csv')
            ], false);
            echo "\n\n";
